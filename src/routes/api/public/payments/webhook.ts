...
async function notify(userId: string, kind: string, title: string, body?: string, data?: Record<string, unknown>) {
  const sb = getSupabase();
  try {
    await (sb.from("notifications") as any).insert({
      user_id: userId, kind, title, body: body ?? null, data: data ?? {}, is_read: false,
    });
  } catch (e) {
...
async function handleSubscriptionUpsert(subscription: any, env: StripeEnv) {
  ...
  if (meta.kind === "twinly_plus") {
    ...
    if (!prev) {
      await notify(userId, "twinly_plus_active", "Twinly+ is active", "You now get 10% off tips, ad-free browsing, and the Twinly+ badge.");
    }
    return;
  }
  ...
  if (isNew) {
    ...
    await notify(userId, "subscription_changed",
      ...
      `Now on the ${tierLabel(tier)} tier.`, { creatorId, tier });
    if (creator?.user_id) {
...
async function handleSubscriptionDeleted(subscription: any, env: StripeEnv) {
  ...
  if (meta.kind === "twinly_plus") {
    ...
    if (meta.userId) await notify(meta.userId, "twinly_plus_ended", "Twinly+ ended", "You can resubscribe anytime from the Pricing page.");
    return;
  }
...
async function handleCheckoutCompleted(session: any, env: StripeEnv) {
  ...
  if (kind === "ppv") {
    ...
    try {
      ...
      await notify(userId, "unlock_purchased", "Unlock complete", "You've unlocked new content. Enjoy!", { assetId });
      const { data: creator } = await (sb.from("creators") as any).select("user_id").eq("id", creatorId).maybeSingle();
      if (creator?.user_id) {
        await notify(creator.user_id, "content_unlocked", "Someone unlocked your content", undefined, { fanId: userId, assetId, amountCents: amount });
      }
  ...
  if (kind === "tip") {
    ...
    try {
      ...
      const creatorName = creator?.stage_name ?? creator?.handle ?? "Creator";
      await notify(userId, "tip_sent", `Tip sent to ${creatorName}`, `$${(amount / 100).toFixed(2)}. Thanks for supporting creators!`);
      if (creator?.user_id) {
        const { data: fan } = await (sb.from("profiles") as any).select("display_name").eq("id", userId).maybeSingle();
        await notify(creator.user_id, "tip_received", `${fan?.display_name ?? "A fan"} tipped $${(amount / 100).toFixed(2)}`, undefined, { fanId: userId, amountCents: amount });
      }
